...
export default function ProgramStudiTable({ data, setNotif, setRefresh }) {
  const table = ["Program Studi", "Fakultas", ""];
  ...
  const deleteHandlerTrue = async () => {
    ...
    if (showDel.show && showDel.id) {
      setLoadingBtn(true);
      try {
        const result = await deleteProgramStudi(token, showDel.id);
            ...
            {data?.map((value) => {
              return (
                ...
                >
                  ...
                  <th className={barisTabel()}>{value.fakultas.nama}</th>
