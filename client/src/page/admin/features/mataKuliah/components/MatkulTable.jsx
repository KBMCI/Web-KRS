...
import { deleteMatkul } from "../services/deleteMatkul";

export default function MatkulTabel({ data, setNotif, setRefresh }) {
  const table = [
    "Kode MK",
    "Nama Mata Kuliah",
    "SKS",
  ...
  const deleteHandlerTrue = async () => {
    ...
    if (showDel.show && showDel.id) {
      setLoadingBtn(true);
      try {
        const result = await deleteMatkul(token, showDel.id);
            ...
            {data?.map((value) => {
              return (
                ...
                >
                  ...
                  <th className={barisTabel()}>{value.program_studi.nama}</th>
